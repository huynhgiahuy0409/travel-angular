...
export class JourneyDetailComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.params.subscribe((routeParams) => {
      let journeyPostId = routeParams.id;
      if (journeyPostId) {
        this.journeyPostResponse$ =
          this.journeyPostService.findById(journeyPostId);
        this.journeyPostResponse$.subscribe((journeyPost) => {
          this.defaultAvatarUrl = this.directLinkService.getDefaultAvatarURL(
            journeyPost.user.gender
          );
        });
      } else {
  ...
  selectDay(index: number) {
    ...
    let selectDayPlacesElement = document.getElementById(
      `${this.journeyDayPlaces}${index}`
    );
